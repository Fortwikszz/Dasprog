...
enshoes = [None]*101 #inisiasi id sepatu yang terkena virus/enkripsi
reshoes = [""]*101 #inisiasi id sepatu yang sudah dipulihkan/dekripsi
...
for i in range(n):
    for j in range(i+1, n):
        if (f"{reshoes[i][:-1]}" == f"{reshoes[j][:-1]}") and (f"{reshoes[i][-1]}" != f"{reshoes[j][-1]}"): #jika angka pada ukuran sepatu sama dan huruf kapital berbeda(L/R)
            ...
            reshoes[j] = "0"                                                                                #dan mengganti salah satu variabel yang sama sehingga tidak terhitung
                                                                                                            #kembali pada perulangan berikutnya
